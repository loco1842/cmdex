...
- CI's `test` job and the `test-windows` job both run `go test -race ./...` (not just `go test ./...`) and block the run on failure, in addition to the `test` job's Playwright e2e suite. `golangci-lint` and `pnpm lint` also block the `typecheck` job on any finding. `make check`/`make fmt`/`make lint` are local-only conveniences — CI does not invoke the Makefile directly.

## Known Cross-Platform Gaps (Windows)

The Windows PTY backend itself is real and CI-verified (see Tests above), but a few adjacent, pre-existing issues remain — found while implementing it, deliberately left unfixed as out of scope for that work:
...
- **Command dispatch assumes `\n` submits a line** (`execution_service.go`, `ExecutionService.RunCommand`). Windows shells (cmd.exe, PowerShell) generally expect `\r` to submit; `\n` alone may not. Doesn't affect interactive typing (xterm.js sends the right bytes), only "run this saved command in the terminal."
- **`shellQuoteDir` (`executor.go`) always uses POSIX single-quote escaping** for the `cd '<dir>' && ...` prefix `RunCommand` builds when a command has a configured working directory. This is meaningless to cmd.exe/PowerShell — a real cross-platform gap in command construction, not a PTY/terminal-I/O issue. `TestRunCommand_FinalCmdWithWorkingDir`/`FinalCmdMultilineScript` are Windows-skipped for exactly this reason.
