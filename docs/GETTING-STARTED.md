...
This project pins Wails to **`v3.0.0-beta.16`** (as specified in `go.mod`). Installing a different version of `wails3` can cause binding generation failures or unexpected behavior at runtime. Verify your installed version:

```bash
wails3 version
```

If it does not match `v3.0.0-beta.16`, reinstall the exact version:
